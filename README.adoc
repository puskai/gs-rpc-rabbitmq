...
The `Client` is a simple POJO that defines a methods for calling the service. When you register it, you can name it anything you want.
...
The bean defined in the `listenerAdapter()` method is registered as a message listener in the container defined in `container()`. It will listen for messages on the "remoting.queue" queue. Because the `Client` class is a POJO, it needs to be wrapped in the `MessageListenerAdapter`, where you specify it to invoke `receiveMessage`.
